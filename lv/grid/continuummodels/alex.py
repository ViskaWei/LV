...
from lv.util.array_filters import *


from lv.fit.legendre import Legendre
from lv.fit.alexsigmoid import AlexSigmoid
...
class Alex(ContinuumModel):
    ...
    def __init__(self, orig=None, trace=None):
        ...
        if isinstance(orig, Alex):
            ...
            self.wave_min = 3000
            self.wave_max = 14000
            self.HYDROGEN_LIMITS = [3646.0, 8203.6, 14584]                   # all numbers in air
...
#region Limits and mask
    ...
    def get_limit_wavelengths(self):
        ...
        limits = np.array(self.HYDROGEN_LIMITS)
        limits = limits[(self.wave_min < limits) & (limits < self.wave_max)]
        limits = np.array([self.wave_min, *Alex.air_to_vac(limits), self.wave_max])
        return limits
